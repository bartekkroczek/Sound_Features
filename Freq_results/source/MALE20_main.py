...
def cmp_vol(soa: int, sound: mixer.Sound, ans_lbs: List[visual.TextStim], feedback: bool) -> [float, bool, str]:
    ...
    standard_quieter = soa > 0

    if standard_first:
        first_volume, second_volume = conf.VOLUME, conf.VOLUME + soa
    else:
        second_volume, first_volume = conf.VOLUME + soa, conf.VOLUME
    ...
    if not timeout:
        if standard_first and standard_quieter:
            corr = (key[0] == conf.SECOND_SOUND_KEY)
        elif standard_first and (not standard_quieter):
            corr = (key[0] == conf.FIRST_SOUND_KEY)
        elif (not standard_first) and standard_quieter:
            corr = (key[0] == conf.FIRST_SOUND_KEY)
        elif (not standard_first) and (not standard_quieter):
            corr = (key[0] == conf.SECOND_SOUND_KEY)
